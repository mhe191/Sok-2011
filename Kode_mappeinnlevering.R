...
df <- subset(df_growth2019, select = c("avg_gdpgrowth", "avg_gi", "avg_nsy", "avg_nry", "avg_gx", "avg_educ", "avg_p", "gdppc0"))
#Gi variablene navn som gir mening. NB: rekkefølgen må være lik rekkefølgen til variablene i df.
labs <- c("Vekst i BNP pc %", "Vekst i investeringer %", "Sparing som andel av BNI %", " reduksjon i naturressurser %", "Vekst i eksport", "Gjennomsnittlig år på skole", "Vekst i befolking %", "Bnp per innbygger(2000)")

#Lag tabellen
st(df, labels=labs)
st(df, vars =c("avg_gdpgrowth", "avg_gi","avg_nsy", "avg_nry", "avg_gx", "avg_educ", "avg_p", "gdppc0"))
st(df, labels=labs,
   summ = list(
...
p_growth_gi <- df_growth2019 %>% 
  ...
  ggtitle("Vekst i investeriger og BNP per cap")

p_growth_gi

# vekst i BNP per innbygger og sparing
df_growth2019 %>% 
  ggplot(aes(x = avg_nsy, y = avg_gdpgrowth)) +
  geom_text(aes(label=iso3c),size=3)+
  scale_color_gdocs() + 
  xlab("Sparing % andel av BNI") + 
  ylab("Gjennomsnittlig årlig vekst i BNP per innbygger 2000-2019") + 
  ggtitle("Vekst i investeriger og Sparing")

# vekst i BNP per innbygger og utdanning
df_growth2019 %>% 
  ggplot(aes(x = avg_educ, y = avg_gdpgrowth)) +
  geom_text(aes(label=iso3c),size=3)+
  scale_color_gdocs() + 
  xlab("Anatll år med utdanning") + 
  ylab("Gjennomsnittlig årlig vekst i BNP per innbygger 2000-2019") + 
  ggtitle("Vekst i investeriger og år med utdanning")


# BNP per innbygger og sparing
df_growth2019 %>% 
  ggplot(aes(x = avg_nsy, y = log(gdppc))) +
  geom_text(aes(label=iso3c),size=3)+
  scale_color_gdocs() + 
  xlab("Sparing % andel av BNI") + 
  ylab("Naturlig logarytmen av BNP per innbygger") + 
  ggtitle("BNP per innbygger og sparing")

# BNP per innbygger og befolkingsvekst
df_growth2019 %>% 
  ggplot(aes(x = avg_p, y = log(gdppc))) +
  geom_text(aes(label=iso3c),size=3)+
  scale_color_gdocs() + 
  xlab("Gjennomsnittlig befolkningsvekst 2000-2019") + 
  ylab("Naturlig logarytmen av BNP per innbygger") + 
  ggtitle("BNP per innbygger og befolkningsvekst")


# BNP per innbygger og utdanning
df_growth2019 %>% 
  ...
  ylab("Naturlig logarytmen av BNP per innbygger") + 
  ggtitle("BNP per innbygger og humankapital")


# Sjekk om det er ekstreme observasjoner i data
model1 <- lm(avg_gdpgrowth  ~ avg_gi + avg_nsy +avg_nry + avg_gx + avg_educ + avg_p + poptot + gdppc0,data=  df_growth2019 ) #NB: Du må legge inn "Dine" variabler her
summ(model1) 
...
#Q1nry <- quantile(df$avg_nry, .25 )
#Q3nry <- quantile(df$avg_nry, .75)
#IQRnry <- IQR(df$avg_nry)

no_outliers1 <- subset(df, df$avg_gi > (Q1gi - 1.5*IQRgi) & df$avg_gi < (Q3gi + 1.5*IQRgi)) #&  df$avg_n > (Q1n - 1.5*IQRn) & df$avg_n < (Q3n + 1.5*IQRn))
#View (no_outliers1)
model2 <- lm(avg_gdpgrowth  ~ avg_gi + avg_nsy +avg_nry + avg_gx + avg_educ + avg_p  + gdppc0, data= no_outliers1) #NB: Du må legge inn "Dine" variabler her
summary(model2)

labs2 <- c("Vekst i BNP pc %", "Vekst i investeringer %", "Sparing som andel av BNI %", " reduksjon i naturressurser %", "Vekst i eksport", "Gjennomsnittlig år på skole", "Vekst i befolking %", "Bnp per innbygger(2000)")

# Jeg kan deretter gå videre å se om det er andre ekstreme observasjoner i andre variabler. 
tab_model(model2, p.style = "stars", pred.labels =labs2, title = "Bestemelsesfaktorer for økonomsik vekst")
...
gapminder %>% 
  ...
  ylab("BNP per innbggger ") + 
  ggtitle("BNP per innbygger og forventet levealder") +
  theme_bw()


data <- read_csv("~/Desktop/2019.csv") %>% 
  select(country = "Country or region", Score)

data2 <- df_growth2019 %>% 
  select(country, year, gdppc)

data3 <- merge(data, data2, by="country")

data3 %>%
  ggplot(aes(Score, gdppc)) +
  geom_text(aes(label=country),size=3)+
  scale_color_gdocs() + 
  xlab("Livskvalitet") + 
  ylab("BNP per innbggger ") + 
  ggtitle("BNP per innbygger og livskavlitet") +
  theme_bw()
